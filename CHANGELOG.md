...
+ [修复] 修复火山方舟与 Seedance 协议合并后视频服务缺少模块导出导致页面无法加载的问题。

+ [优化] 本地视频合并改为顺序读取片段并显示读取进度，连接本地服务或读取片段失败时会指出具体片段和服务地址。

+ [调整] Seedance 原生、OpenAI 兼容和中转视频生成任务的状态查询间隔统一为 30 秒。
+ [修复] 渠道请求地址按协议补齐版本路径，Seedance 中转裸地址改为调用 `/api/v3/contents/generations/tasks`，不再错误进入 `/v1`。
+ [新增] 渠道协议新增 Seedance 原生模式，视频生成可通过原生任务接口可靠控制声音、时长、水印和参考媒体。
+ [新增] Windows 本地媒体服务支持代理 Deepgram 识别本地原视频并用字词时间戳校准反推分镜口播，无需公网媒体地址。
+ [调整] 视频反推新增原片人声口播区间，分镜配置和音频时间轴可持续查看原片口播、新 TTS 对齐、生成时长及未匹配原因。
+ [新增] Seed TTS 2.0 音频模型可选择 OpenAI 兼容或 New API V3 时间戳模式，原生语音合成支持保存字词时间戳供字幕、动画和镜头精确对齐。
+ [调整] 音频节点可查看 Seed TTS 字词时间轴并提示应用结果，视频反推和 TTS 回写的视频时长统一限制为 4-15 秒，成片合成优先使用原生时间戳并在匹配失败时回退停顿分析。
+ [修复] 音频配置重复生成时不再把上游旁白反复写回提示词，避免 TTS 输入持续累积并生成超长循环音频。
+ [修复] Seed TTS 2.0 音频生成按模型提供火山引擎音色并发送对应 `Voice_type`，不再错误使用 OpenAI 的 `alloy` 音色。
+ [新增] 音频调用脚本新增“豆包语音合成”模板，通过火山引擎 v3 单向流式接口生成语音，音色 ID 填在“声音指令”中。
+ [新增] 视频合并支持选择一条统一 TTS 旁白，按 FFmpeg 检测到的停顿生成句级时间轴、裁剪或补齐画面，并创建成片时间轴节点。
+ [调整] 视频反推生成的新分镜配置默认关闭 Seedance 原生声音，改由固定音色和语速的独立音频配置生成整条旁白。
+ [优化] 本地媒体服务限制远程视频下载大小并阻止本机、内网及不安全重定向地址。
+ [修复] 本地视频合并正确保留带流标识和语言元数据的源音轨，并校验前端选择数量与服务实际收到的片段数量，避免静音或缺段成片。
+ [修复] 本地视频合并改由 Windows 媒体服务代理下载跨域远程视频，避免火山 TOS 等地址在浏览器中返回 `Failed to fetch`。
+ [新增] Windows 本地媒体服务支持将画布中多选的视频统一为 1080×1920、30fps、H.264/AAC 后按顺序拼接并创建完整成片节点，无需安装 Codex。
+ [调整] 选择 OpenAI 协议的自定义中转渠道时，Seedance 视频模型使用 `/v1/videos` 提交和查询任务，并从 `metadata.url` 下载生成视频。
+ [修复] 豆包长视频理解改用 SSE 流式响应并提高分镜输出上限，避免浏览器长时间无响应或 JSON 被截断。
+ [新增] 视频节点支持通过可配置视频理解脚本反推长视频文案和分镜，并创建可编辑的逐镜头视频生成流程。
+ [调整] 渠道模型可为同一模型分别配置文本调用脚本和视频理解脚本，文本模型也可作为视频理解模型使用。
+ [调整] 豆包系列模型作为视频理解模型时，默认通过 `/v1/chat/completions` 的 `video_url` Base64 输入完整视频，并移除不支持 MP4 的 OpenAI `input_file` 视频模板。
+ [调整] 完整移除百度统计支持，网站统计仅保留可选的 Google Analytics 4。
